...
/**
 * Legacy GET wrapper used by the existing `fetchDataExport(http:
 * ClarityHttp, ...)` helper and the `ClarityProvider` class (deleted in
	...
	async get(
		...
		if (!response.ok) {
			throw new ProviderApiError(
				...
				`Clarity ${path} returned HTTP ${response.status}`,
			);
		}
		return parsed;
	}
...
/**
 * Backward-compat alias. The old per-provider `ClarityApiError` class is
 * gone; consumers (worker processor's quota detector at
 * `apps/worker/src/processors/provider-fetch.processor.ts:146` does an
 * `instanceof ClarityApiError && (status === 402 || status === 429)`
 ...
 * new code paths — `ProviderApiError` thrown by `ClarityHttpClient` will
 * also satisfy `instanceof ClarityApiError`. Phases 5/6 retire the
 * worker code path; Phase 7 can drop this alias when no callers remain.
 */
export const ClarityApiError = ProviderApiError;
export type ClarityApiError = ProviderApiError;

/**
 * Adapter that lets the existing `fetchDataExport(http: ClarityHttp, ...)`
 * helper call through `ClarityHttpClient` instead of `ClarityHttp`. The
 * manifest path uses this so a single `BaseHttpClient` instance handles
 * auth, timeouts and error wrapping; the helper keeps its current
 * signature.
 *
 * The shim flattens multi-value queries (Clarity's `dimension1/2/3` come
 * in as `string | string[]`) into the path's query string before calling
 * `BaseHttpClient.get`, whose `query: Record<string, string>` parameter
 ...
 * `ClarityHttp.get`. Normalise so callers see the same shape regardless
 * of which path produced the value.
 */
export const buildLegacyShim = (client: ClarityHttpClient, ctx: FetchContext): ClarityHttp =>
	({
		get: async (
			path: string,
			query: Record<string, string | string[]>,
