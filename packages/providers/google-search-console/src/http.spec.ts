...
import { GoogleSearchConsoleHttpClient, GscApiError } from './http.js';
...
describe('GoogleSearchConsoleHttpClient', () => {
	...
	it('mints a token and sends the bearer header on a successful POST', async () => {
		...
		const client = new GoogleSearchConsoleHttpClient(baseConfig);
	...
	it('throws ProviderApiError preserving the upstream status on non-OK responses', async () => {
		...
		// Alias check: GscApiError === ProviderApiError, so instanceof works.
		await expect(client.post('/forbidden', {}, {}, stubContext())).rejects.toBeInstanceOf(GscApiError);
	});
